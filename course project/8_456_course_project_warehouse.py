'''
Программа "Склад"
Реализованно: Сlass WarehouseOfficeEquipment (класс склада где выполняются следующие 9 функций):
1.Добавить технику на склад take_to_warehouse(self, *info)
2. Найти технику на складе по номеру модели(например, laserJet1) find_in_warehouse(self, name_office_equipment, number_model)
3. Количество техники на складе в наличии по типу(принтеров: 2 штуки) sum_items_warehouse(self, name_office_equipment)
4. Показать, вывести весь список техники на складе look_all_warehouse(self)
5.Удалить технику со склада по модели(например, laserJet1) delete_from_warehouse(self, number_model)
6.Сохранить в файл список техники со склада save_to_file(self, name_file_write)
7.Удаление файла списка техники(спрашивает подтверждение для удаления файла и делает бекап удаляемого файла перед удалением, на всякий случай)
delete_file(self, name_file_write)
8.Беэкап файла списка техники
backup_data(self, name_file_read, name_file_write_backup)
9.Техническая информация о программе с проверкой хэшом создателя(необходим файл author.txt в той же директории)

Class OfficeEquipment - родительский для класса Printer
Сlass Printer(OfficeEquipment) - дочерний класс класса OfficeEquipment
Реализованно:
1. формирование списка информации о принтере для добавления на склад
При входе запрашивает имя кладовщика(ответственного лица, добавляющего технику на склад)
его пароль(пароль - 1), хэш-пароля хранится в pwd.txt
(есть 3 попытки ввода, со sleep 10 секунд между неудачными попытками и окончание работы если пароль не верен)
Для работы программы необходимо:
1. 8_456_course_project_warehouse.py
2. pwd.txt
3. author.txt
Так же есть .exe файл warehouse27.exe
'''
...
class WarehouseOfficeEquipment:
    ...
    def hash_password(password):
        try:
            ...
            print('error hash_password() class "WarehouseOfficeEquipment"')
...
class OfficeEquipment:
    def __init__(self, name_office_equipment):
        try:
            ...
            print('Error! __init__ class "OfficeEquipment"')
...
class Printer(OfficeEquipment):
    def __init__(self, name_office_equipment, number_model, serial_number):
        try:
            ...
            print('Error! __init__ class "Printer(OfficeEquipment)"')
    ...
    def add_printer(self):
        try:
            ...
            print('Error! add_printer() class "Printer(OfficeEquipment)"')
...
while login_attempts != 0:
    try:
        ...
        print('Error login_attemps main()')
while menu_input != 'q':
    if menu_input == '1':
        try:
    ...
    if menu_input == '7':
        try:
            ...
            are_you_sure = input('Are you sure DELETE office equipments? y - confirm ')
            if are_you_sure == 'y':
                order_1.backup_data(name_file_to_delete + '.txt', name_file_to_delete + '_backup.txt')
